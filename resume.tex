...
\linespread{1.05}  %!p(string=Software engineer with 5 years building production distributed systems and ML infrastructure at scale. Experienced in load balancing, service mesh operations, and data pipeline optimization. Track record of zero-downtime migrations and cost reduction while maintaining strict availability SLAs.)alladio needs more leading (space between lines)
...
\footnotesize
\textbf{Summary:} Software engineer with 5 years building production distributed systems and ML infrastructure at scale. Experienced in load balancing, service mesh operations, and data pipeline optimization. Track record of zero-downtime migrations and cost reduction while maintaining strict availability SLAs.
\end{minipage}
...
\cvheadingstart
  \cvcompany{Lyft Inc.}{New York, NY}
    \cvrole{\textbf{Software Engineer, Production Infrastructure \& Marketplace Forecasting}}{11/2021 - Present}
        ...
        \cvitemend
        \cvproject{Demand Forecasting Improvements: Scaled ML pipeline to support 30x increase in model granularity for marketplace pricing.}{}
        \cvitemstart
          \cvitem{Implemented compression and pruning across data pipeline, reducing over-the-network bandwidth by 80\%.}
          \cvitem{Collaborated with data scientists to validate model performance, ensuring accuracy (MAPE < 10\%) throughout pipeline optimization.}
          \cvitem{Implemented model inference handlers managing data processing, imputation, and post-inference aggregation.}
        \cvitemend
